...
const SponsorGoalToggle = (props: {
  ...
  const [current, setCurrent] = useState(props.defaultValue || props.options[0]?.value);
...
const UseCaseCarousel = (props: {
  ...
  const handlePrev = () =>
    setIndex((prev) => (prev > 0 ? prev - 1 : props.useCases.length - 1));
  const handleNext = () =>
    setIndex((prev) => (prev < props.useCases.length - 1 ? prev + 1 : 0));
...
const SponsorEngagementForm = (props: {
  sponsorType?: string; // user selection
  useCases: { title: string; description: string }[];
}) => {
  const [goal, setGoal] = useState(props.sponsorType || "poc");

  return (
    <div className="space-y-4 p-3 bg-bg-main text-text-primary rounded-md">
      <SponsorGoalToggle
        title="I want:"
        options={[
